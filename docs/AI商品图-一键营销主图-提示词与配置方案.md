...
- `品类基础约束`
  用于先说明所有营销主图都必须保持商品真实结构、材质、颜色和比例。

- `品类规则正文`
  必须单独输出当前命中的品类规则 `prompt`，不能只写“当前商品品类为...”。

- `平台规则正文 / required / forbidden`
  平台 `prompt` 是平台语境说明；
  `required` 转成“必须满足：...”；
  `forbidden` 转成“禁止：...”。
...
- `valuePrompt 扩展段`
  必须紧跟在参数行后面，把每个字段值对应的执行型描述展开。

- `通用负向约束`
  用于补足平台规则没有覆盖的营销主图通用风险。

- `质量要求`
  统一收束营销主图的主体识别、卖点层级、文案可读和合规边界。
...
产品类型={productType}；场景背景={sceneBackground}；平台信息={platformInfo}；商品信息={productInfo}；视觉风格={visualStyle}；营销元素={marketingElements}；文案语种={copyLanguage}。

valuePrompt 扩展段拼装顺序如下：
1. `sceneBackground.valuePrompt`
2. `productInfo.valuePrompt`
3. `visualStyle.valuePrompt`
4. `marketingElements.valuePrompt`
5. `copyLanguage.valuePrompt`

按以上顺序拼接后的 valuePrompt 文本：
[场景背景] {sceneBackgroundValuePrompt}
[商品信息] {productInfoValuePrompt}
[视觉风格] {visualStyleValuePrompt}
[营销元素] {marketingElementsValuePrompt}
[文案语种] {copyLanguageValuePrompt}

通用负向约束：禁止改变商品真实结构、颜色和材质；禁止虚构商品功效、夸大促销利益点或制造误导性前后对比；禁止新增与售卖内容不一致的赠品、功能元素或品牌资产；禁止使用大面积水印、Logo、二维码、联系方式、边框或覆盖性贴片遮挡主体；禁止因过强特效、滤镜、光效、描边或装饰元素导致商品主体失真、难以识别或卖点不可读。
...
## 5.3 `valuePrompt` 的实际插入位置

`valuePrompt` 必须放在参数行之后、通用负向约束和质量要求之前。
...
品类规则：突出版型、面料与穿搭卖点，文案区域不要遮挡关键轮廓。

平台规则正文：适配淘宝高密度信息浏览，允许营销表达但避免牛皮癣式堆叠，主体优先。
...
[场景背景] 构建与商品使用语义一致的场景，避免违和和伪造使用状态。
[商品信息] 完整信息层级表达，注意主次分明，避免版面拥挤。
[视觉风格] 强化潮流表达与色彩张力，保持商品真实轮廓。
[营销元素] 折扣标识位置克制，避免遮挡商品主体。
[文案语种] 中文文案简洁直达，层级清晰。
...
```ts
const paramLine = `产品类型=${productType}；场景背景=${sceneBackground}；平台信息=${platformInfo}；商品信息=${productInfo}；视觉风格=${visualStyle}；营销元素=${marketingElements}；文案语种=${copyLanguage}。`;

const valuePromptLines = [
  sceneBackgroundValuePrompt && `[场景背景] ${sceneBackgroundValuePrompt}`,
  productInfoValuePrompt && `[商品信息] ${productInfoValuePrompt}`,
  visualStyleValuePrompt && `[视觉风格] ${visualStyleValuePrompt}`,
  marketingElementsValuePrompt && `[营销元素] ${marketingElementsValuePrompt}`,
  copyLanguageValuePrompt && `[文案语种] ${copyLanguageValuePrompt}`
].filter(Boolean).join("\n");

const genericNegativePrompt = `通用负向约束：禁止改变商品真实结构、颜色和材质；禁止虚构商品功效、夸大促销利益点或制造误导性前后对比；禁止新增与售卖内容不一致的赠品、功能元素或品牌资产；禁止使用大面积水印、Logo、二维码、联系方式、边框或覆盖性贴片遮挡主体；禁止因过强特效、滤镜、光效、描边或装饰元素导致商品主体失真、难以识别或卖点不可读。`;
