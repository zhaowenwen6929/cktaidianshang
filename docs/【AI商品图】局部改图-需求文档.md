...
## 端到端使用流程
1. 用户上传素材。
2. 系统基于上传图片内容自动识别默认 `podPartialEditCategory`，用户也可手动切换。
...
+ 当前功能存在自动默认品类逻辑，来源为上传图片内容识别
+ 该逻辑用于预填 `podPartialEditCategory`，帮助命中对应品类规则
...
+ 图片识别结果只用于默认值，不应覆盖用户手动改写结果
+ 当前实现没有像印花图裂变那样的“手动锁定 ref”，如果上传内容变化导致重新识别，前端会更新品类
...
## 三个关键能力的提示词配置
### 图片识别获取信息
当前功能的品类默认值应来自图片识别；若用户手动切换，则以后者为准。
...
3. `podPartialEditCategory` 的来源应为“图片自动识别”或“用户手动选择”，不应依赖文件名关键词推断。
