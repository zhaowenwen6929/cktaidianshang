...
- 品类来源优先级：`productCategory` > 用 `productType` 命中 `categoryRulesByTool[*].aliases` 归一 > `通用品类`。
2. 读取平台规则：`platformRulesByTool[platformLabel]`，重点拼接“可作为附图/补充图使用”的角色约束，以及 `required + forbidden`。
3. 读取品类规则：`categoryRulesByTool[normalizedCategory]`，拼接商品局部结构、材质和特写边界。
4. 读取高级字段值扩展：按 `productType / displayType` 命中值追加 `valuePrompt`，不要只输出参数值。
5. 细节图字段分层必须固定：
- `productCategory` 决定局部焦点是否合理，例如数码类看接口/模组，服饰类看面料/缝线。
- `productType` 决定具体可放大的结构对象。
- `displayType` 决定画面是偏纯细节、工艺放大还是细节与功能关系联动。
6. 约束优先级必须固定：`platform forbidden` > `category hard prompt` > `platform required` > `displayType` > `productTypePrompt` > `supplement`。
7. 若平台主图规则严格，细节图必须明确降级为“附图/补充图”，禁止在提示词里暗示该图可直接替代主图。
8. `displayType=细节 + 功能关联` 时，允许表现局部与功能价值的关系，但不得生成伪透视爆炸图、虚构内部结构或夸大效果图。
9. `resolution` 仅在高级模式稳定传入；普通模式没有分辨率时，`outputSpec` 自动降级为 `输出比例={ratio}；输出数量={count}。`
10. `supplement` 为空时整段删除；若补充内容与平台/品类约束冲突，按高优先级规则裁掉冲突语义。
11. 安全兜底：最终串必须包含“局部特写清晰准确、材质真实、边缘干净、不虚构结构”的语义。
...
| `displayTypePrompt` | `displayType` 命中值 | 本文第 4 章 `optionValueExpansionsByTool` | 决定特写与功能关系表达 |
...
平台约束：适配亚马逊商品图库语境，细节图应作为补充图片而非主图使用，重点展示特征细节、材质、接口、尺寸或使用关联细节；必须真实准确，不伪造效果，不添加误导性图形、Logo 或水印，不让局部细节与实际售卖商品不一致。 必须满足：作为附图使用、细节真实、主体一致、结构可辨。 禁止：替代主图、虚构结构、误导性功能图、额外水印Logo。

品类约束：当前商品品类为「家电数码类」，请保持该品类应有的真实结构、材质、颜色与比例，不改变 SKU 核心特征。 重点关注：接口按键、材质边界、模组结构、功能可信。

高级设置：产品类型=蓝牙耳机；展示形式=细节 + 功能关联。

选项扩展约束：重点放大耳机腔体、耳帽、充电触点、转轴、网孔和材质边界，避免增加不存在的接口。在细节特写的基础上，明确表现该局部与功能/使用价值的关系，但不夸大功效。
...
1. `platformLabel=亚马逊` 先注入“仅作为附图/补充图使用”的硬约束，避免误解成主图。
2. `productCategory=家电数码类` 用来锁定局部结构真实性，防止特写中接口、网孔、转轴等被修错。
3. `displayType=细节 + 功能关联` 允许在特写之外补一层“该局部解决什么使用问题”的表达，但不能变成夸张功能海报。
4. `supplement` 只做低优先级补充；若用户写“拆开内部结构展示芯片”“做爆炸剖面图”，执行层应自动压制。
...
## 8.2 AI Assist 回填高级字段（当前代码已配提示词，但前端默认关闭入口）
