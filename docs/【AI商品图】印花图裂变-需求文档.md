...
+ `podVariationShape`：仅 `艺术设计 / 文字强化 / 通用` 生效
...
裂变参数：模式={podVariationMode}；品类={podVariationCategory}；出图数量={podVariationOutputCount}。
选项扩展约束：
...
1. 主链路字段不只是 `mode`，还包括 `category / referenceStrength / divergence / backgroundColor / burstContent / content / shape / outputCount`。
2. 不同模式下的生效字段不同，开发不能做成“所有字段固定拼装”。
3. 当前唯一自动推断能力是基于文件名关键词的品类默认值，不是大模型识别。
4. 当前没有平台规则、没有补充说明、没有显式 AI 帮写，因此文档应围绕“模式规则 + 品类规则 + 条件化字段扩展 + 固定拼装顺序”实现。 
